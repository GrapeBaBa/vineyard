 ...
 *
 * This load balancer uses a naive failure detector that removes the host on the first connection
 * failure. The intention here is just to demonstrate how to write complex load balancing logic
 * using lower level constructs in the client.
 *
...
public class TcpLoadBalancer<W, R> extends RoundRobinLoadBalancer<W, R> {
  ...
  public static <W, R> ConnectionProvider<W, R> create(Observable<SocketAddress> hosts) {
    return ConnectionProvider.create(connectionFactory -> new TcpLoadBalancer<>(hosts,
        connectionFactory));
  }
}
